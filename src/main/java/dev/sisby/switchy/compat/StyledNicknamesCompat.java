...
public class StyledNicknamesCompat {
	public static SwitchyComponentType.Builder<String> nameComponent(SwitchyComponentType.Builder<String> b) {
		return b
			...
			.playerReader((p, id) -> Objects.requireNonNullElse(PlayerDataApi.getGlobalDataFor(p, Identifier.tryBuild("stylednicknames", "nickname"), StringTag.TYPE), StringTag.valueOf(id)).getAsString())
			.playerMutator((v, p) -> {
				PlayerDataApi.setGlobalDataFor(p, Identifier.tryBuild("stylednicknames", "nickname"), StringTag.valueOf(v));
				PlayerDataApi.setGlobalDataFor(p, Identifier.tryBuild("stylednicknames", "permission"), ByteTag.valueOf(false));
			})
